 ...
 * status (ui/widget.ts) is the one UI surface that renders it. Before this
 * bus, every mode module imported updateStatus from ui/widget while
 ...
 * registers the status updater as the listener (onStatusChange). A test
 * (or an embed without a UI) that registers no listener gets a no-op.
 */
...
/** Mode modules call this on every state change that affects the status. */
