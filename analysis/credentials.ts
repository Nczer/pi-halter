/**
 * Credential path detection for bash commands — a raw-text scan.
 *
 * Deliberately independent of the tree-sitter parse: it runs on the command
 * string itself, before/without analysis, so a parse failure can never hide
 * a credential read (fail-closed). Path operands are resolved through
 * path-analysis (resolvePathReal + the warned pattern check) — the only
 * dependency on the path side. Everything else is text-level credential
 * semantics: heredoc/comment stripping (data, not operands), quote-aware
 * word splitting, glob-shape decoding, and bare-symlink checks.
 *
 * There is one outcome, not two: every detection is a WARNING (3.26.0 — the
 * hard-block tier was removed, see config/path-rules.ts). A warning is still
 * absolute in effect: it kills auto-allow (SafetyRule) and stops every
 * automatic mode (gate/dspa-gate.ts), so a credential path always reaches a
 * human prompt carrying the very-high-risk note.
 *
 * Interface:
 *  - checkCommandForCredentialPaths(command, cwd) — the single entry point:
 *    the matched warn rule (pattern name, resolved path, or the
 *    GLOB_UNVERIFIED marker), or null. Accumulation prefers a concrete
 *    credential match over an unverifiable-glob marker.
 *  - checkBareRelativeTokens(tokens, cwd) — bare relative tokens (plain
 *    strings or QuotedToken) whose cwd symlink escapes cwd or points at a
 *    credential (must run even when the string pre-scan early-returns: a
 *    symlink's literal name carries no credential text). Quoted words are
 *    probed as single literals — never operator-split or glob-expanded.
 *  - GLOB_UNVERIFIED_PREFIX — the warn marker a failed glob-expansion probe
 *    raises (an honest "unverifiable glob" stop, not a credential match).
 *  - CREDENTIAL_SCAN_RE — the fast pre-scan regex (FastAllowRule re-runs it
 *    on the dequoted form).
 *  - stripHeredocBodies / stripShellComments — text preprocessing (tests
 *    exercise them directly; the scanner is the only production caller).
 */
import path from "node:path";
import fs from "node:fs";
import { warnPaths } from "../config";
import { logGlobVerifyError } from "../config/logging";
import { expandTilde } from "./path-util";
import { tokenizeSegmentQuoted, type QuotedToken } from "./tokenizer";
import {
  isChildOf,
  isPathWarnedResolved,
  resolvePathReal,
} from "./path-analysis";


/**
 * Pre-compiled regex for fast credential pattern detection in bash commands.
 * Loose match — false positives just trigger the more expensive resolve+check.
 * Matches credential file/dir name roots: .ssh, .env, .aws, .docker/config.json,
 * standalone keyfile basenames (id_rsa), .envrc, and *.pem file names.
 */
export const CREDENTIAL_SCAN_RE = /\.(?:ssh|gnupg|gpg|vault|secret|secrets|env|envrc|aws|gcloud|azure|git-credentials|hg|netrc|npmrc|pypirc|docker|pem)\b|\bid_(?:rsa|ed25519|ecdsa|dsa)\b/;

/**
 * Marker prefix for a warn raised by a FAILED GLOB-EXPANSION PROBE (the
 * bare-symlink check could not verify what a relative glob reaches —
 * globSync threw / is unavailable, or the expansion exceeds the probe cap).
 * It is NOT a matched credential pattern: the display sites (dspa-gate
 * stop reason, prompt-builder line) branch on the prefix and render an
 * honest "unverifiable glob" stop.
 */
export const GLOB_UNVERIFIED_PREFIX = "glob-unverified:";

export function isGlobUnverified(value: string | null | undefined): boolean {
  return typeof value === "string" && value.startsWith(GLOB_UNVERIFIED_PREFIX);
}

export function globUnverifiedToken(value: string | null | undefined): string {
  return typeof value === "string" ? value.slice(GLOB_UNVERIFIED_PREFIX.length) : "";
}

/**
 * Strip heredoc BODIES from a command so credential scanning doesn't flag
 * credential-looking tokens that are merely DATA fed to stdin (the body of
 * `cat > x <<'EOF' … .ssh/id_rsa … EOF`). A real credential READ is always in
 * the command line (operand, redirect target), never in the body.
 *
 * Conservative by design: only the common form is stripped — the heredoc
 * operator ends the logical line (optionally followed by a comment or a
 * pipeline/background operator). Quoted pseudo-heredocs mid-line, here-strings
 * (`<<<`), and UNTERMINATED heredocs are left untouched (fail-closed: the
 * text is still scanned; an unterminated heredoc also fails the tree-sitter
 * parse and prompts anyway).
 *
 * A line is only treated as a heredoc START when bash unambiguously starts one:
 * `<<` is a standalone word (a glued `x<<EOF` is a literal word in bash), it
 * is not inside a comment, and it is not inside an unterminated quoted string.
 * When in doubt the body is KEPT and still scanned — every decline is
 * fail-closed, so a false start can never hide a live command line.
 */
export function stripHeredocBodies(cmd: string): string {
  const HEREDOC_START_RE = /<<(?!<)(-?)\s*(['"]?)([A-Za-z0-9_][A-Za-z0-9_.-]*)\2\s*(?:[#|&;].*)?$/;
  /** Return the heredoc delimiter if this line unambiguously starts one, else null. */
  const findHeredocStart = (line: string): string | null => {
    const m = line.match(HEREDOC_START_RE);
    if (!m) return null;
    const opIdx = line.lastIndexOf("<<");
    // `<<` must be a standalone word: start of line or preceded by whitespace.
    // Glued forms (x<<EOF, =<<EOF) are literal words in bash, not redirects.
    if (opIdx > 0 && !/\s/.test(line[opIdx - 1])) return null;
    const before = line.slice(0, opIdx);
    // A word-boundary `#` before the operator puts it inside a comment —
    // bash starts no heredoc there (covers `# c <<EOF` and `cmd;# c <<EOF`).
    if (/^#|[\s;|&(]#/.test(before)) return null;
    // Unbalanced unescaped quotes before the operator mean it sits inside a
    // multi-line string — literal text, not a redirect.
    const dq = (before.match(/(?<!\\)"/g) ?? []).length;
    const sq = (before.match(/(?<!\\)'/g) ?? []).length;
    if (dq % 2 || sq % 2) return null;
    return m[3];
  };
  const lines = cmd.split("\n");
  const drop = new Set<number>();
  const pending: string[] = [];
  const bodyStart = new Map<string, number>();
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (pending.length > 0) {
      const first = pending[0];
      if (line.trim() === first) {
        pending.shift();
        const start = bodyStart.get(first) ?? i;
        bodyStart.delete(first);
        for (let j = start; j < i; j++) drop.add(j);
      }
      continue;
    }
    const delim = findHeredocStart(line);
    if (delim) {
      if (!pending.includes(delim)) bodyStart.set(delim, i + 1);
      pending.push(delim);
    }
  }
  if (drop.size === 0) return cmd;
  return lines.filter((_, i) => !drop.has(i)).join("\n");
}

/**
 * Mask shell comments (word-boundary `#` to end of physical line) so the
 * credential scan doesn't flag credential-looking text that is merely a
 * comment (`# rotate the .env tomorrow\nls` must not block `ls`). Comments
 * never execute, so masking them can only remove false positives — a real
 * credential operand stays on a live line and is still scanned.
 *
 * Quote- and continuation-aware, mirroring bash:
 *  - `#` starts a comment only at a WORD START: line start, or after
 *    whitespace / `;|&(`. `x#y`, `${v#pat}`, `VAR=#x`, `echo "a"# b` keep
 *    their `#` (literal word content).
 *  - Inside '…' or "…" a `#` is literal; quotes may span lines.
 *  - A backslash-escaped `#` (`\#`) is literal.
 *  - Backslash+newline outside quotes splices the logical line: a `#` on the
 *    continuation line is judged against the character BEFORE the splice
 *    (`foo \<nl># x` → `foo # x` → comment; `foo\<nl># x` → `foo# x` →
 *    literal). A comment ends at its physical line even when it ends in `\\`
 *    (comments do not continue).
 *
 * Comment characters are replaced with spaces (offsets and newlines
 * preserved) so downstream line-based preprocessing sees the same shape.
 */
export function stripShellComments(cmd: string): string {
  const out = cmd.split("");
  let inSingle = false;
  let inDouble = false;
  let escaped = false;
  let comment = false;
  let prev = "\n"; // last significant char of the LOGICAL line (word-start check)
  for (let i = 0; i < cmd.length; i++) {
    const c = cmd[i];
    if (comment) {
      if (c === "\n") { comment = false; prev = "\n"; }
      else { out[i] = " "; }
      continue;
    }
    if (escaped) { escaped = false; continue; }
    if (inSingle) {
      if (c === "'") { inSingle = false; prev = c; }
      continue;
    }
    if (inDouble) {
      if (c === "\\") { escaped = true; continue; } // escapes next (incl. newline splice)
      if (c === '"') { inDouble = false; prev = c; }
      continue;
    }
    if (c === "'") { inSingle = true; prev = c; continue; }
    if (c === '"') { inDouble = true; prev = c; continue; }
    if (c === "\\") {
      if (cmd[i + 1] === "\n") {
        i++; // line splice: the logical line continues; `prev` keeps the
        // character before the backslash (word-start judged against it)
      } else {
        escaped = true;
      }
      continue;
    }
    if (c === "#" && (/[ \t\n;|&(]/.test(prev))) {
      comment = true;
      out[i] = " ";
      continue;
    }
    prev = c;
  }
  return out.join("");
}

/**
 * Check bare relative tokens for symlinks in cwd that point outside it.
 * A malicious repo can ship a symlink (often absolute, e.g. `link →
 * /home/<user>/.ssh/id_rsa` — the username is frequently in git history) so
 * `cat link` reads a credential although the command line only shows a
 * benign name. The literal name carries no credential text, so the string
 * pre-scan in checkCommandForCredentialPaths would early-return without ever
 * resolving it, and FastAllowRule passes bare relative tokens through.
 *
 * Only SYMLINKS are resolved: a regular file in cwd cannot escape cwd, and a
 * directory symlink used as `cat linkdir/f` contains a "/" so it goes
 * through the normal path resolution, which already follows the parent chain.
 * Quoted names are covered too — quoting prevents glob expansion but not
 * symlink following (`cat "link"` still reads the target).
 *
 * QUOTED WORDS ARE ONE SHELL WORD: their internal `;`/`&&`/`|` are data, not
 * operators (a `node -e 'a; b'` body is not a command chain), so quoted
 * tokens are never operator-split, and their quoted glob chars never expand
 * at runtime (`cat "l*"` is the literal name l*) — no expansion probe; the
 * word is probed as a single literal name. A glob char OUTSIDE the quoted
 * spans (`a*b'c'`) still expands: the quote-stripped word is exactly its
 * expansion pattern, so that case keeps the expansion probe. Plain strings
 * (no quoting facts) take the full unquoted treatment — fail-closed.
 *
 * Relative GLOBS are expanded at gate time and every match probed the same
 * way as a bare name: bash expands `cat l*` at runtime, so a shipped symlink
 * hiding behind a glob is the same threat as the literal name (the name-
 * decode glob check in checkCommandForCredentialPaths covers only the glob's
 * SPelling against credential patterns, never its filesystem expansion).
 */
export function checkBareRelativeTokens(
  tokens: Array<string | QuotedToken>,
  cwd: string,
): string | null {
  let warned: string | null = null;
  /** Keep a warn rule: a concrete credential match outranks an
   *  unverifiable-glob marker, otherwise the first finding wins. */
  const keep = (rule: string | null): void => {
    if (!rule) return;
    if (!warned || (isGlobUnverified(warned) && !isGlobUnverified(rule))) warned = rule;
  };
  let cwdReal: string;
  try { cwdReal = fs.realpathSync(cwd); } catch { cwdReal = path.resolve(cwd); }

  /**
   * Probe one existing path: a symlink whose real target (or one-level
   * textual target) matches a credential pattern → warned with that pattern;
   * escapes cwd → warned with the real target (same approval bar as reading
   * the target directly). Regular files/dirs are trusted (they cannot escape
   * their location).
   */
  const probePath = (candidate: string, linkDir: string): void => {
    let st: fs.Stats;
    try { st = fs.lstatSync(candidate); } catch { return; }
    if (!st.isSymbolicLink()) return;
    // Resolve the target two ways:
    //  - real: follow the chain via realpath (works when the target exists;
    //    catches credential names anywhere along the chain)
    //  - lex: one-level textual readlink (works for DANGLING links, where
    //    realpath cannot follow — a link to a not-yet-existing id_rsa is
    //    still the attack shape and must be gated). A relative target
    //    resolves against the link's own directory.
    const real = resolvePathReal(candidate, cwd);
    let lex: string | null = null;
    try {
      const tgt = fs.readlinkSync(candidate);
      lex = path.isAbsolute(tgt) ? path.resolve(tgt) : path.resolve(linkDir, tgt);
    } catch { /* target vanished mid-flight — real path already checked */ }
    const candidates = [real, lex].filter((r): r is string => !!r);
    // Credential patterns first: the honest reason to show the user, ahead of
    // the generic "link leaves the working directory" form.
    for (const resolved of candidates) {
      const warnedResult = isPathWarnedResolved(candidate, resolved);
      if (warnedResult.warned) { keep(warnedResult.matchedRule); return; }
    }
    for (const resolved of candidates) {
      if (isChildOf(resolved, cwdReal)) continue;
      // Symlink escapes cwd → same approval bar as reading the target path
      // directly (the file tool resolves symlinks the same way).
      keep(resolved);
      return;
    }
  };

  /**
   * A relative glob token: bash expands it at runtime, so the set it can
   * reach is its cwd-anchored expansion — probe every match like a bare
   * name. Only plain glob characters are expanded: runtime expansions
   * ($/`) and brace/extended groups are not statically expandable (the path
   * layer keeps their prefixed forms opaque), and absolute/~/..-anchored
   * patterns are already resolved (and failed closed on) by the path layer.
   *
   * Static search-directory guard: glob chars never cross "/", so a pattern
   * whose glob chars all sit AFTER its last "/" searches exactly the literal
   * directory before that slash — and if that directory is missing, the
   * expansion is provably empty: nothing to probe, no prompt. (Patterns with
   * no "/" search cwd itself; patterns with a glob char in the directory part
   * are not statically verifiable and take the fail-closed path below.)
   * Required because some runtimes' fs.globSync throws on no-match (observed
   * in the field on Bun, which is what pi runs): without the guard the
   * everyday no-match glob would fail closed every time.
   *
   * Fail closed on expansion errors or >4096 matches — the same bar as
   * isAllowedRootToken: an unverifiable glob prompts, marked with
   * GLOB_UNVERIFIED_PREFIX so the stop renders honestly (and the error is
   * recorded in .log/glob-err.jsonl for mining).
   */
  const checkGlob = (t: string): void => {
    if (/[$`{}()]/.test(t)) return;
    if (t.startsWith("/") || t.startsWith("~")) return;
    if (/(^|\/)\.\.(\/|$)/.test(t)) return; // .. segment → path layer
    const lastG = Math.max(t.lastIndexOf("*"), t.lastIndexOf("?"), t.lastIndexOf("["));
    const lastSlash = t.lastIndexOf("/");
    const staticDir = lastSlash >= 0 && lastG > lastSlash ? t.slice(0, lastSlash) : null;
    if (staticDir !== null && !fs.existsSync(path.join(cwdReal, staticDir))) return;
    let matches: string[];
    try {
      if (typeof fs.globSync !== "function") {
        throw new Error("fs.globSync unavailable in this runtime");
      }
      matches = fs.globSync(path.join(cwdReal, t));
    } catch (e) {
      logGlobVerifyError(t, e, cwdReal);
      keep(GLOB_UNVERIFIED_PREFIX + t); // can't verify — prompt
      return;
    }
    if (matches.length > 4096) {
      keep(GLOB_UNVERIFIED_PREFIX + t); // too many to verify
      return;
    }
    for (const m of matches) {
      // What the expansion ACTUALLY reaches is checked against the credential
      // patterns. This is the filesystem answer to the question a pure
      // wildcard spelling (`.*/id_rsa`, `*`) cannot give statically — the
      // name-shape check in globComponentName skips such components instead of
      // matching every name there is.
      const resolved = resolvePathReal(m, cwd);
      const hit = isPathWarnedResolved(path.relative(cwdReal, m) || m, resolved);
      if (hit.warned) keep(hit.matchedRule);
      probePath(m, path.dirname(m));
    }
  };

  /** Probe a literal (non-glob) bare name: a symlink whose target warns. */
  const checkLiteral = (t: string): void => {
    if (!t || t === "." || t === "..") return;
    if (t.startsWith("-")) return;
    if (t.includes("/")) return; // literal with a slash → path layer
    // NOTE: tokens containing `=` are NOT skipped — a cwd file/symlink can
    // literally be named `a=b` (the shell reads it as a plain filename in
    // argument position). The probe is existence-gated (lstat), so a real
    // `K=V` assignment or `--flag=value` that names no file no-ops.
    probePath(path.join(cwdReal, t), cwdReal);
  };

  const checkOne = (t: string): void => {
    // Globs are expanded and probed below — bash expands them at runtime,
    // so a shipped symlink behind a glob is the same threat as the name.
    if (/[*?\[\]]/.test(t)) { checkGlob(t); return; }
    checkLiteral(t);
  };

  // Skip token 0 (the command name — a bare name is looked up in PATH, not
  // cwd). Unquoted tokens: mirror the scanner's operator split so
  // `cat link;ls` and `cat link>x` still see the bare name. A WHOLE quoted
  // word (content entirely from quoted spans) is ONE shell word: no operator
  // split (internal operators are data), no expansion probe for its globs
  // (they never expand), but the word itself is probed as a literal name
  // (quoting does not prevent symlink following — `cat "link"` reads the
  // target). A partly quoted word keeps the split: its unquoted operators
  // really are syntax.
  for (let i = 1; i < tokens.length; i++) {
    const raw = tokens[i];
    const tok = typeof raw === "string"
      ? { text: raw, quoted: false, unquotedGlob: false, quotedWhole: false }
      : raw;
    if (tok.quotedWhole) {
      if (tok.unquotedGlob) checkGlob(tok.text);
      checkLiteral(tok.text);
      continue;
    }
    const parts = tok.text
      .split(/[;&|<>]+/)
      .map(p => p.replace(/\)+$/, ""))
      .filter(Boolean);
    for (const p of parts) checkOne(p);
  }
  return warned;
}

/* ─── Glob-shape credential detection ──────────────────────────────────────
 *
 * A glob can spell a credential name without typing it out (`s*sh`, `id_rs?`,
 * `*.pem`). Three rules keep that check honest in both directions — it must
 * not miss a credential, and it must not invent one:
 *
 * 1. Only glob characters OUTSIDE quotes expand, so only a word that is not
 *    wholly quoted is a candidate — see QuotedToken.quotedWhole. A quoted
 *    word is a pattern or a literal name, never a file set (`grep ".*" f`).
 * 2. A component is compared against credential NAMES only when it types out
 *    at least one literal LETTER. A component that is pure wildcard noise
 *    (`.`, `[0-9]*`, `*.*`) compiles to something like `^..*$`, which matches
 *    every name in existence — that is not a detection, it is a refusal
 *    waiting to happen to a benign command.
 * 3. What a pure-wildcard component reaches is therefore a FILESYSTEM
 *    question, and is answered that way: checkBareRelativeTokens expands every
 *    relative glob and matches the real result against the credential
 *    patterns. A dot-glob before an `id_rsa` operand is flagged when a
 *    directory named like a credential is actually there; `echo .*` is not
 *    flagged for a name the glob never touched.
 *
 * Together these are the 3.26.0 false-positive fix. The reported case
 * was a `sed` substitution whose program body (`s|/…|/N|`) was operator-split
 * at every `|` into fake path parts, one of which (`[0-9]*`, `.*`) then
 * compiled into a near-universal wildcard and "matched" a credential name.
 */

/** Cap on `{a,b}` variants expanded from one component, above which the token is unverifiable. */
const BRACE_VARIANT_CAP = 64;
/** Sentinel: the component spells a glob that cannot be translated (see globComponentName). */
const GLOB_UNTRANSLATABLE = "\u0000";

/** Index of the `}` closing the `{` at `start` (nesting + `\` honoured), or -1 when unbalanced. */
function findBraceClose(text: string, start: number): number {
  let depth = 0;
  for (let i = start; i < text.length; i++) {
    const c = text[i]!;
    if (c === "\\") { i++; continue; }
    if (c === "{") depth++;
    else if (c === "}" && --depth === 0) return i;
  }
  return -1;
}

/** Split a brace group's body at its top-level commas. */
function braceAlternatives(body: string): string[] {
  const alts: string[] = [""];
  let depth = 0;
  for (let i = 0; i < body.length; i++) {
    const c = body[i]!;
    if (c === "\\") {
      alts[alts.length - 1] += c + (body[i + 1] ?? "");
      i++;
      continue;
    }
    if (c === "," && depth === 0) { alts.push(""); continue; }
    if (c === "{") depth++;
    else if (c === "}") depth = Math.max(0, depth - 1);
    alts[alts.length - 1] += c;
  }
  return alts;
}

/**
 * Expand `{a,b}` alternation in one path component into the names it spells.
 * A brace group WITHOUT a top-level comma is not alternation — `{2,4}` is a
 * regex quantifier, `{a}` a literal, an unbalanced `{` literal text — and
 * stays literal characters. Returns null when the form cannot be expanded
 * statically (nesting past 4 levels, or more than BRACE_VARIANT_CAP variants).
 */
function braceVariants(comp: string, depth = 0): string[] | null {
  if (depth > 4) return null;
  let prefix = "";
  for (let i = 0; i < comp.length; i++) {
    const c = comp[i]!;
    if (c === "\\") { prefix += c + (comp[i + 1] ?? ""); i++; continue; }
    if (c !== "{") { prefix += c; continue; }
    const close = findBraceClose(comp, i);
    const alts = close < 0 ? null : braceAlternatives(comp.slice(i + 1, close));
    if (!alts || alts.length < 2) { prefix += "{"; continue; } // literal brace group
    const tail = braceVariants(comp.slice(close + 1), depth + 1);
    if (tail === null) return null;
    const out: string[] = [];
    for (const alt of alts) {
      for (const t of tail) {
        out.push(prefix + alt + t);
        if (out.length > BRACE_VARIANT_CAP) return null;
      }
    }
    return out;
  }
  return [prefix];
}

/** Index of the `]` closing the `[` at `start` (bash rules), or -1 when unterminated. */
function classEndIndex(text: string, start: number): number {
  let j = start + 1;
  if (text[j] === "!") j++;
  if (text[j] === "]") j++; // a `]` first in the class body is literal content
  for (; j < text.length; j++) {
    if (text[j] === "\\") { j++; continue; }
    if (text[j] === "]") return j;
  }
  return -1;
}

/** Escape one character for literal use in a regex. */
function escapeReChar(c: string): string {
  return /[.*+?^${}()|[\]\\]/.test(c) ? "\\" + c : c;
}

/**
 * Translate one brace-free glob variant into an anchored regex source. Only
 * the metachars bash treats as globbing are special: `*`, `?`, `[...]`
 * (leading `!` negation, leading `]` as content, an unterminated `[` as a
 * literal character) and `\x` escapes. Returns null when the variant cannot be
 * translated — a runtime expansion (`$`, backtick) inside the globbing text,
 * or a dangling backslash — which the caller reports as unverifiable.
 */
function globToRegexSource(v: string): string | null {
  let out = "";
  for (let i = 0; i < v.length; i++) {
    const c = v[i]!;
    if (c === "*") { out += "[^/]*"; continue; }
    if (c === "?") { out += "."; continue; }
    if (c === "\\") {
      const next = v[i + 1];
      if (next === undefined) return null; // dangling backslash
      out += escapeReChar(next);
      i++;
      continue;
    }
    if (c === "[") {
      const end = classEndIndex(v, i);
      if (end < 0) { out += "\\["; continue; } // unterminated → literal `[`
      let body = v.slice(i + 1, end);
      let neg = false;
      if (body.startsWith("!") && body.length > 1) { neg = true; body = body.slice(1); }
      const cls = body.replace(/[\\\]^]/g, (m) => "\\" + m);
      out += `[${neg ? "^" : ""}${cls}]`;
      i = end;
      continue;
    }
    if (c === "$" || c === "`") return null; // runtime value inside a glob: unverifiable
    out += escapeReChar(c);
  }
  return `^${out}$`;
}

/**
 * The credential name this globbing component can denote (s*sh → the ssh
 * directory name, id_rs? → id_rsa), the GLOB_UNTRANSLATABLE sentinel when its
 * spelling cannot be checked, or null. Applies the literal-letter floor of the
 * section comment above.
 */
function globComponentName(comp: string): string | null {
  const variants = braceVariants(comp);
  if (variants === null) return GLOB_UNTRANSLATABLE;
  for (const variant of variants) {
    const skeleton = variant.replace(/\[[^\]]*\]/g, "").replace(/[*?]/g, "");
    if (!/[a-zA-Z]/.test(skeleton)) continue; // pure wildcard noise, see above
    const src = globToRegexSource(variant);
    if (src === null) return GLOB_UNTRANSLATABLE;
    let re: RegExp;
    try { re = new RegExp(src); } catch { return GLOB_UNTRANSLATABLE; }
    for (const pattern of warnPaths) if (re.test(pattern)) return pattern;
  }
  return null;
}

/**
 * Glob-shape check for one word: the credential name it can expand to, the
 * GLOB_UNVERIFIED marker when a static component cannot be translated, or
 * null. A token with a runtime expansion in it (`$VAR`, `$(…)`, backticks)
 * is not a static glob at all — bash substitutes first, so its static
 * spelling decides nothing (`${cmds[@]}` stays auto-allow). The FS-probe
 * path (checkBareRelativeTokens) skips the same tokens.
 */
const globRuleForWord = (token: string): string | null => {
  if (token.includes("$") || token.includes("`")) return null;
  for (const comp of token.split("/")) {
    if (!/[*?[\]{}]/.test(comp)) continue;
    const found = globComponentName(comp);
    if (found === GLOB_UNTRANSLATABLE) return GLOB_UNVERIFIED_PREFIX + token;
    if (found) return found;
  }
  return null;
};

/**
 * Check a bash command string for credential path references. Returns the
 * matched warn rule — a credential pattern name, a resolved path (a symlink
 * leaving the working directory), or the GLOB_UNVERIFIED marker — or null when
 * nothing credential-shaped is reachable. See the file header for what a warn
 * does (never auto-allowed, always a prompt).
 */
export function checkCommandForCredentialPaths(command: string, cwd: string): string | null {
  // Heredoc bodies and shell comments are DATA, not path operands — strip
  // them before scanning so credential names in the body / a comment don't
  // false-positive (`# check the credential dir\nls` must not flag `ls`).
  const scanCmd = stripHeredocBodies(stripShellComments(command));

  // Quote-aware words (quote chars stripped, so a dequoted credential name is
  // still visible — a quote-splitting bypass). The quoting facts decide the
  // two questions the raw text cannot answer on its own:
  //   operator split — `;`, `&`, `|`, `<`, `>` are shell syntax only OUTSIDE
  //     quotes, so a wholly quoted word (a sed/awk program, a regex) is ONE
  //     operand and must never be cut into fake path parts;
  //   glob expansion — quoted glob chars never expand, so a wholly quoted
  //     word is checked literally only, while a partly quoted word keeps the
  //     expansion checks (`s*h'x'`).
  const words = tokenizeSegmentQuoted(scanCmd);
  const partsOf = (w: QuotedToken): string[] =>
    w.quotedWhole
      ? [w.text]
      : w.text.split(/[;&|<>]+/).map((p) => p.replace(/\)+$/, "")).filter(Boolean);

  // Fast pre-scan: if no credential pattern appears anywhere in the command,
  // skip the precise per-token pass. Checked on the dequoted and
  // backslash-stripped forms too, so quote-splitting (.en''v) and
  // backslash-splitting (s\sh) cannot hide the pattern text. Glob characters
  // defeat the string regex (s?sh ≠ the name), so a globbing command always
  // takes the precise pass.
  const dequoted = words.map((w) => w.text).join(" ");
  const unstripped = scanCmd.replace(/\\/g, "");
  const hasGlob = /[*?[\]{}]/.test(scanCmd);

  // Bare-token symlink check — must run even when the pre-scan below would
  // early-return (a symlink's literal name carries no credential text).
  let warned = checkBareRelativeTokens(words, cwd);

  /** Keep a finding; a concrete credential match outranks an unverifiable glob. */
  const keep = (rule: string | null): void => {
    if (!rule) return;
    if (!warned || (isGlobUnverified(warned) && !isGlobUnverified(rule))) warned = rule;
  };

  if (
    !hasGlob &&
    !CREDENTIAL_SCAN_RE.test(scanCmd) &&
    !CREDENTIAL_SCAN_RE.test(dequoted) &&
    !CREDENTIAL_SCAN_RE.test(unstripped)
  ) {
    return warned;
  }

  // Valid env-var name pattern: starts with letter/underscore, only alphanumeric/underscore.
  // Flags like `--output=path` have a leading dash, so they won't match.
  const ENV_VAR_NAME_RE = /^[a-zA-Z_][a-zA-Z0-9_]*$/;

  /** Check a path string against the credential patterns, returning the matched rule. */
  const checkPath = (pathStr: string): string | null => {
    const resolved = resolvePathReal(expandTilde(pathStr), cwd);
    return isPathWarnedResolved(pathStr, resolved).matchedRule;
  };

  /** Check one token (or value) against the credential path patterns. */
  const checkToken = (t: string): string | null => {
    if (!t) return null;

    // Handle --flag=value syntax
    if (t.startsWith("-") || t.startsWith("--")) {
      const eqIdx = t.indexOf("=");
      if (eqIdx > 0 && eqIdx < t.length - 1) {
        const value = t.slice(eqIdx + 1);
        if (value) return checkPath(value);
      }
      return null;
    }

    // Env assignments (FOO=bar, FOO=/path): check the value, never skip it.
    // Pure assignment statements produce no parsed segments, so the value is
    // invisible to AST-based analysis; a credential path staged in a variable
    // (`X=the ssh dir && cat $X`) would otherwise auto-allow — the later
    // `$VAR` use is just a non-path token to the scanner. The assignment is the
    // only statically visible moment, so it must prompt by itself.
    const eqIdx = t.indexOf("=");
    if (eqIdx !== -1) {
      const beforeEquals = t.slice(0, eqIdx);
      if (ENV_VAR_NAME_RE.test(beforeEquals)) {
        const value = t.slice(eqIdx + 1);
        if (value) return checkPath(value);
        return null; // FOO= (empty value) — nothing to check
      }
    }

    return checkPath(t);
  };

  // Pre-split every token on shell operators: whitespace-only tokenization
  // leaves operators stuck to or inside tokens: `cat cred;`, `cat cred|grep`,
  // `cat cred>/tmp/x`, `X=cred&& ls`, `$(cat cred)`. Unquoted, a path can
  // never contain `;`, `&`, `|`, `<`, `>`, or end with `)` — there they are
  // always shell syntax — so check every part. Without this, `cat cred; ls`
  // evades the scan entirely (the token is `cred;`). Words whose content is
  // entirely quoted are the exception: their operators are data, and cutting
  // them up invents path operands that were never there.
  for (const word of words) {
    for (const token of partsOf(word)) {
      // Check original token
      keep(checkToken(token));

      // Also check the token with backslashes stripped to prevent
      // backslash-splitting bypasses (e.g. an s\sh spelling of the ssh
      // directory). The tokenizer preserves backslashes outside quotes, so we
      // handle it here.
      if (token.includes("\\")) keep(checkToken(token.replace(/\\/g, "")));

      // Also check the glob-expanded shape (s*sh → the ssh directory name,
      // id_rs? → id_rsa). Skipped for a wholly quoted word: quoted globs
      // cannot expand at runtime (`grep ".*" f` is a pattern, `cat "s*sh"` a
      // literal name), while unquoted ones keep the check (`cat */id_rsa`).
      if (!word.quotedWhole) keep(globRuleForWord(token));
    }
  }

  return warned;
}
