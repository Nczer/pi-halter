...
- **Judge modes** — `/dspa` auto-allows operations that pass a deterministic hard floor *and* a two-stage LLM-judge verdict (stateless pass, then an intent pass with reasoning-blind session context) (visible toast); when a judge call fails (model unreachable, or no verdict within the deadline), a repeatable "Judge again" re-runs the intent pass; `/dspat` runs both judge stages on every prompt (bash/file/tool) and shows the final verdict (stage 2, else stage 1), recording agreement stats; both fail toward the prompt. The modes are one machine — **manual / dspa / dspat / dsp**: enabling one leaves the others off (switching resets the left judge mode's session stats). `/dspa` is the only session-persistent mode — its toggle is stored in `settings-ext.json` (`halter.mode`) and restores on session start and `/reload`; `/dsp` (full bypass) and `/dspat` are session-scoped by design
...
- **Decision log** — `/halter-decision-log` records every gate decision to a JSONL file (off by default; see *Configuration → Decision log*); three small diagnostic ledgers (`.log/unresolved.jsonl`, `.log/judge.jsonl`, `.log/glob-err.jsonl`) are on by default, toggled together by `/halter-ledger-log`
...
- Three small ledgers are **on by default** (toggled together — `/halter-ledger-log [on|off]`, same arg handling as `/halter-decision-log`; D17 split; same 5 MiB rotation each): `.log/unresolved.jsonl` records the fate of each path token static analysis couldn't bind (`outcome: prompted | gate-stop | auto-allowed`, the LLM resolver's suggested dirs, the user's decision, whether it became a confirmed resolution — convergence is the token flipping `prompted` → `auto-allowed`, D12 — positional parameters (`$1`, `$@`) never converge: the store refuses to key a grant on a call-site-local token); `.log/judge.jsonl` records judge signal only: stage-1/stage-2 verdict disagreements (`diff`), stage failures (`infra`: `no-model` / `no-auth` / `no-explanation` / `call-failed` — judge OFF stays silent), and D13 stage-2 path mismatches (`paths`); `.log/glob-err.jsonl` records glob-verify failures only — a healthy run writes nothing (a failed glob re-check: the pattern, the error, the cwd). Test-only env seams win over the toggles: `HALTER_UNRESOLVED_LOG=off`, `HALTER_JUDGE_LOG=off`, `HALTER_GLOBERR_LOG=off`
