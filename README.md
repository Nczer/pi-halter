...
- **DSP floor** (`dspa-gate.ts`) — deterministic hard floor for `/dspa`: parse errors, obscured command positions, credential patterns, network egress, outside-base paths (unbound paths are resolved from the command when possible before gating — D7), the rm carve-out. Everything else is judgeable — including file writes into session-granted dirs (D3: the grant exempts the dir, the two-stage judge decides the content). Judges the analysis carried on the prompt — one tree-sitter parse per decision
