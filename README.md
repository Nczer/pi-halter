...
- **Decision log** — `/halter-decision-log` records every gate decision to a JSONL file (see *Configuration → Decision log*); two small diagnostic ledgers (`.log/unresolved.jsonl`, `.log/judge.jsonl`) are always-on
...
- Two small ledgers are **always-on** (the `/halter-decision-log` toggle covers `decisions.jsonl` only — D17 split; same 5 MiB rotation each): `.log/unresolved.jsonl` records the fate of each path token static analysis couldn't bind (`outcome: prompted | gate-stop | auto-allowed`, the LLM resolver's suggested dirs, the user's decision, whether it became a confirmed resolution — convergence is the token flipping `prompted` → `auto-allowed`, D12), and `.log/judge.jsonl` records judge signal only: stage-1/stage-2 verdict disagreements (`diff`), stage failures (`infra`: `no-model` / `no-auth` / `no-explanation` / `call-failed` — judge OFF stays silent), and D13 stage-2 path mismatches (`paths`). Test-only env seams: `HALTER_UNRESOLVED_LOG=off`, `HALTER_JUDGE_LOG=off`
