...
export interface Store {
  ...
  /** Check if a file path is auto-allowed. */
  hasAllowedPath(path: string): boolean;
  ...
  /** Get a copy of all auto-allowed paths. */
  listAllowedPaths(): Set<string>;
...
export function createStore(nowFn = Date.now): Store {
  ...
  return {
    ...
    hasAllowedPath(p) { return paths.has(p); },
    ...
    addAllowed(rules) {
      ...
      rules.paths?.forEach(p => paths.add(p));
    ...
    listAllowedPaths() { return new Set(paths); },
